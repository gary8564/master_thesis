# -*- coding: utf-8 -*-
"""
Created on Sun Aug  9 11:37:49 2020
...
test = np.hstack((first_test, third_test, fifth_test, eighth_test,test_input)) # vary in accordance with d.o.f. of training dataset
...
plt.show()
#%% save input 
# orgainze data to ccwtarx
test_input = testy.reshape((1,-1))
first_test = first_test[:len(testy),:]
second_test = second_test[:len(testy),:]
third_test = third_test[:len(testy),:]
fourth_test = fourth_test[:len(testy),:]
fifth_test = fifth_test[:len(testy),:]
sixth_test = sixth_test[:len(testy),:]
seventh_test = seventh_test[:len(testy),:]
eighth_test = eighth_test[:len(testy),:]
first_test = first_test.reshape((1,-1))
second_test = second_test.reshape((1,-1))
third_test = third_test.reshape((1,-1))
fourth_test = fourth_test.reshape((1,-1))
fifth_test = fifth_test.reshape((1,-1))
sixth_test = sixth_test.reshape((1,-1))
seventh_test = seventh_test.reshape((1,-1))
eighth_test = eighth_test.reshape((1,-1))
first = np.squeeze(first_test)
second = np.squeeze(second_test)
third = np.squeeze(third_test)
fourth = np.squeeze(fourth_test)
fifth = np.squeeze(fifth_test)
sixth = np.squeeze(sixth_test)
seventh = np.squeeze(seventh_test)
eighth = np.squeeze(eighth_test)
kobe = np.squeeze(yhat)
test_input = np.squeeze(test_input)
rel_eighth = eighth-kobe
rel_seventh = seventh-kobe
rel_sixth = sixth-kobe
rel_fifth = fifth-kobe
rel_fourth = fourth-kobe
rel_third = third-kobe
rel_second = second-kobe
rel_first = first-kobe
rel_eighth1 = eighth-test_input
rel_seventh1 = seventh-test_input
rel_sixth1 = sixth-test_input
rel_fifth1 = fifth-test_input
rel_fourth1 = fourth-test_input
rel_third1 = third-test_input
rel_second1 = second-test_input
rel_first1 = first-test_input
d1 = {'Eigth':rel_eighth, 'Fifth':rel_fifth,
      'Third':rel_third, 'First':rel_first, 'Input':kobe}
df1 = pd.DataFrame(data=d1)
df1.to_csv('kobe_ccwtarx(predict)_mlp.csv', index=False, header=False)
d2 ={'Eigth':rel_eighth1, 'Fifth':rel_fifth1,
     'Third':rel_third1,'First':rel_first1, 'Input':test_input}
df2 = pd.DataFrame(data=d2)
df2.to_csv('kobe_ccwtarx(real)_mlp.csv', index=False, header=False)

#%% save results
model.save("mlp_future_predict_past.h5")
