...
def stateless_model(epochs, output_dim):
    ...
    model.add(LSTM(32, activation='tanh',input_shape=(n_steps, n_features),# kernel_initializer='he_uniform', 
...
'''
# stateful lstm
model = stateful_model(epochs,batch_size_stateful, trainy.shape[1])
'''
'''
# Bidrectional lstm
...
#%% TEST (health(if kobe skip this cell))
# summarize model.
model.summary()
# load test data 
# test
...
#%% save train error and test error
train_error=train_predict-trainy
np.save('train_error(lstm)',train_error)
test_error=yhat-testy
np.save('test_error(lstm)',test_error)
#%% save model
model.save('lstm_nonlineaer.h5')
