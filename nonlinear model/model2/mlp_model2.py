...
def mlp_model(epochs, n_input, output_dim):
    model = Sequential()
    model.add(Dense(64, input_dim = n_input, kernel_initializer='he_uniform'))
    #model.add(BatchNormalization())
    model.add(PReLU())
    #model.add(Dropout(0.15))
...
test_newzealand = pd.read_excel('NEW ZEALAND 600gal.xlsx')
...
np.save('train_error(mlp)',train_error)
test_error=yhat-testy
np.save('test_error(mlp)',test_error)
#%% save model
model.save("mlp_nonlinear.h5")
