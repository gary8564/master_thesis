# -*- coding: utf-8 -*-
"""
Created on Sat Sep 19 13:56:41 2020
...
def mlp_model(epochs, n_input, output_dim):
    model = Sequential()
    model.add(Dense(64, activation='softplus', input_dim = n_input))#, kernel_initializer='he_uniform'))
    #model.add(BatchNormalization())
    #model.add(PReLU())
    #model.add(Dropout(0.15))
...
corralitosX2 = np.squeeze(corralitosX2)
elcentroX2 = np.squeeze(elcentroX2)
emilyvilleX2 = np.squeeze(emilyvilleX2)
friulliX2 = np.squeeze(friulliX2)
hollisterX2 = np.squeeze(hollisterX2)
jp311X2 = np.squeeze(jp311X2)
kocaeliX2=np.squeeze(kocaeliX2)
turkeyX2 = np.squeeze(turkeyX2)
northridgeX2=np.squeeze(northridgeX2)
romeX2=np.squeeze(romeX2)
sakariaX2=np.squeeze(sakariaX2)
noricaX2 = np.squeeze(noricaX2)
# flatten input
n_input_temp = chichiX1.shape[1] * chichiX1.shape[2]
chichiX1 = chichiX1.reshape((chichiX1.shape[0], n_input_temp))
noricaX1 = noricaX1.reshape((noricaX1.shape[0], n_input_temp))
corralitosX1 = corralitosX1.reshape((corralitosX1.shape[0], n_input_temp))
elcentroX1 = elcentroX1.reshape((elcentroX1.shape[0], n_input_temp))
emilyvilleX1 = emilyvilleX1.reshape((emilyvilleX1.shape[0], n_input_temp))
jp311X1 = jp311X1.reshape((jp311X1.shape[0], n_input_temp))
friulliX1 = friulliX1.reshape((friulliX1.shape[0], n_input_temp))
amberleyX1 = amberleyX1.reshape((amberleyX1.shape[0], n_input_temp))
hollisterX1 = hollisterX1.reshape((hollisterX1.shape[0], n_input_temp))
turkeyX1 = turkeyX1.reshape((turkeyX1.shape[0], n_input_temp))
...
northridgeX1 = northridgeX1.reshape((northridgeX1.shape[0], n_input_temp))
...
chichi_X = np.concatenate((chichiX1, chichiX2), axis=1)
norica_X = np.concatenate((noricaX1, noricaX2), axis=1)
corralitos_X = np.concatenate((corralitosX1, corralitosX2), axis=1)
elcentro_X = np.concatenate((elcentroX1, elcentroX2), axis=1)
emilyville_X = np.concatenate((emilyvilleX1, emilyvilleX2), axis=1)
jp311_X = np.concatenate((jp311X1, jp311X2), axis=1)
friulli_X = np.concatenate((friulliX1, friulliX2), axis=1)
amberley_X = np.concatenate((amberleyX1, amberleyX2), axis=1)
hollister_X = np.concatenate((hollisterX1, hollisterX2), axis=1)
turkey_X = np.concatenate((turkeyX1, turkeyX2), axis=1)
...
northridge_X = np.concatenate((northridgeX1, northridgeX2), axis=1)
...
# 20 cases
trainX = np.vstack((chichi_X,chiayi_X,hualien331_X,meinung_X,california20_X,yilan_X,
                    amberley_X,athens_X,corralitos_X,elcentro_X,emilyville_X,friulli_X,
                    hollister_X,jp311_X,kocaeli_X,turkey_X,northridge_X,rome_X,sakaria_X,norica_X))
...
test_hualien = pd.read_excel('2018HUALIEN.xlsx')
...
test_chichi = pd.read_excel('CHICHI_600gal.xlsx')
...
np.save('train_error_model1(mlp)',train_error)
np.save('test_error_model1(mlp)',test_error) # same hysteretic
# np.save(test_error_model2(mlp),test_error) #different hysteretic
#%% save model and architecture to single file
model.save("mlp_nonlinear_model1.h5")
