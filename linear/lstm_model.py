# -*- coding: utf-8 -*-
"""
Created on Sat Jul 18 19:00:24 2020
...
def bidirectional_model(epochs,output_dim):
    ...
    #model.add(BatchNormalization())
    #model.add(Dropout(rate=0.1))
    #model.addBidireactional((LSTM(64, activation='tanh',
                   #kernel_constraint=MinMaxNorm(min_value=-1.0, max_value=1.0),
                   #bias_constraint=MinMaxNorm(min_value=-1.0, max_value=1.0),
                   #recurrent_constraint=MinMaxNorm(min_value=-1.0, max_value=1.0),
    #               kernel_regularizer=l2(1e-6), recurrent_regularizer=l2(1e-6), 
...
'''
# random search
model = KerasRegressor(build_fn=create_vallina_lstm, verbose=1, epochs=300, batch_size=16)
# define the grid search parameters
#layers=[[8], [8, 4], [8, 8, 4]]
#nodes=[4,8,16,32,64]
params = dict(lr=[0.0001, 0.0003, 0.001, 0.003, 0.01, 0.03, 0.1, 0.3])
...
history = model.fit(trainX, trainy, epochs=epochs, batch_size=batch_size, 
                    validation_split=0.33, verbose=1)#, shuffle=False)
...
'''
# another split time steps method
testX1, testy = split_multisequence(test, n_steps)
testX2 = split_sequence(test_input, n_steps)
print(testX1.shape, testX2.shape, testy.shape)
testX = np.dstack((testX2, testX1))
'''
...
#test1 = np.loadtxt(os.path.join(path,'10%_kobe_responses_1_3_E70%_1_8DOF_500Hz_a.dat')) #10%noises
...
np.save('test_error_health_lstm', error_health)
np.save('test_error_damaged_lstm', error_damaged)