# -*- coding: utf-8 -*-
"""
Created on Tue Oct 20 15:38:06 2020
...
while True:
    try:
        n=int(input('please entry the number of d.o.f (known+unkown target):'))# number of elements
        a = list(map(int,input("\nEnter the specified floor in order(knowns unkown): ").strip().split()))[:n]# read inputs from user using map() function 
...
n_steps = 60
jijiX1, jiji_y = split_multisequence(jiji, n_steps)
noricaX1, norica_y = split_multisequence(norica, n_steps)
mexicoX1, mexico_y = split_multisequence(mexico, n_steps)
berkeleyX1, berkeley_y = split_multisequence(berkeley, n_steps)
christchurchX1, christchurch_y = split_multisequence(christchurch, n_steps)
jp311X1, jp311_y = split_multisequence(jp311, n_steps)
hualien19X1, hualien19_y = split_multisequence(hualien19, n_steps)
amberleyX1, amberley_y = split_multisequence(amberley, n_steps)
nantouX1, nantou_y = split_multisequence(nantou, n_steps)
hualienX1, hualien_y = split_multisequence(hualien, n_steps)
california20X1, california20_y = split_multisequence(california20, n_steps)
hualien331X1, hualien331_y = split_multisequence(hualien331, n_steps)
chileX1, chile_y = split_multisequence(chile, n_steps)
chiayiX1, chiayi_y = split_multisequence(chiayi, n_steps)
yilanX1, yilan_y = split_multisequence(yilan, n_steps)
romeX1, rome_y = split_multisequence(rome, n_steps)
sakariaX1, sakaria_y = split_multisequence(sakaria, n_steps)
athensX1, athens_y = split_multisequence(athens, n_steps)
kocaeliX1, kocaeli_y = split_multisequence(kocaeli, n_steps)
meinungX1, meinung_y = split_multisequence(meinung, n_steps)
nantou602X1, nantou602_y = split_multisequence(nantou602, n_steps)
christchurch11X1, christchurch11_y = split_multisequence(christchurch11, n_steps)
california19X1, california19_y = split_multisequence(california19, n_steps)
train_X=np.vstack((jijiX1,noricaX1,mexicoX1,berkeleyX1,christchurchX1,christchurch11X1,jp311X1,
...
'''
train_X=np.vstack((jijiX1,noricaX1,mexicoX1,berkeleyX1,christchurch11X1,jp311X1,
                   hualien19X1,nantouX1,hualienX1,california20X1))
'''
jiji_y=jiji_y.reshape(-1,1)
norica_y=norica_y.reshape(-1,1)
mexico_y=mexico_y.reshape(-1,1)
berkeley_y=berkeley_y.reshape(-1,1)
christchurch_y=christchurch_y.reshape(-1,1)
jp311_y=jp311_y.reshape(-1,1)
hualien19_y=hualien19_y.reshape(-1,1)
amberley_y=amberley_y.reshape(-1,1)
nantou_y=nantou_y.reshape(-1,1)
hualien_y=hualien_y.reshape(-1,1)
california20_y=california20_y.reshape(-1,1)
hualien331_y=hualien331_y.reshape(-1,1)
chile_y=chile_y.reshape(-1,1)
chiayi_y=chiayi_y.reshape(-1,1)
yilan_y=yilan_y.reshape(-1,1)
rome_y=rome_y.reshape(-1,1)
sakaria_y=sakaria_y.reshape(-1,1)
athens_y=athens_y.reshape(-1,1)
kocaeli_y=kocaeli_y.reshape(-1,1)
meinung_y=meinung_y.reshape(-1,1)
nantou602_y=nantou602_y.reshape(-1,1)
christchurch11_y=christchurch11_y.reshape(-1,1)
california19_y=california19_y.reshape(-1,1)
...
trainy = np.vstack((jiji_y,norica_y,mexico_y,berkeley_y,christchurch_y,christchurch11_y,
                    ...
                    athens_y,kocaeli_y,meinung_y,nantou602_y,california19_y))

print('trainX:',trainX.shape)
print('trainy:',trainy.shape)

np.save('trainX_mlp_158predict3',trainX)
np.save('trainy_mlp_158predict3',trainy)
